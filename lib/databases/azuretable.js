...
//        if (done) done(null); // strange on azure...
  ...
  addEvents: function (events, callback) {
    ...
    async.concat(events,
      function (event, callback) {
        ...
        var results = [
          function (callback) {
            var storedEvent = new StoredEvent(event, self.options.getPartitionKey());
  ...
  getEvents: function (query, skip, limit, callback) {
    ...
    tableQuery = buildEventsQuery(tableQuery, query, self.options.getPartitionKey());
...
var StoredEvent = function (event, partitionKey) {
  this.PartitionKey = eg.Entity(partitionKey);
  this.RowKey = eg.Entity(buildEventRowKey(event));
...
var StoredUndispatchedEvent = function (event) {
  this.PartitionKey = eg.Entity(event.id);
  this.RowKey = eg.Entity(event.id);
  this.aggregateId = eg.Entity(event.aggregateId);
  this.id = eg.Entity(event.id);
  this.context = eg.Entity(event.context);
  this.aggregate = eg.Entity(event.aggregate);
...
var StoredSnapshot = function (snapshot) {
  ...
  this.aggregate = eg.Entity(snapshot.aggregate) || undefined;
  this.context = eg.Entity(snapshot.context) || undefined;
...
function buildEventRowKey(event) {
  return event.aggregateId + '|' + event.id;
}

function buildEventsQuery(tableQuery, query, partitionKey) {
  var length = query.aggregateId.length - 1;
  var lastChar = query.aggregateId[length];
  var nextLastChar = String.fromCharCode(lastChar.charCodeAt(0) + 1);

  var startsWithEndPattern = query.aggregateId.substring(0, length) + nextLastChar;

  return tableQuery
    .where('PartitionKey eq ?', partitionKey)
    .and('RowKey ge ?', query.aggregateId)
    .and('RowKey lt ?', startsWithEndPattern);
}
